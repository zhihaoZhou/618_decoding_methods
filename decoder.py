...
class Decoder(nn.Module):
    ...
    def top_k_caption(self, img_features, beam_size, k):
        ...
        while True:
            ...
            top_preds, top_words = output[0].topk(k, 0, True, True)
            indices = torch.multinomial(top_preds, beam_size)
            top_preds = top_preds[indices]
            top_words = top_words[indices]
            # if step == 1:
            #     top_preds, top_words = output[0].topk(beam_size, 0, True, True)
            # else:
            #     top_preds, top_words = output.view(-1).topk(beam_size, 0, True, True)
    ...
    def nucleus_caption(self, img_features, beam_size, cumulative_prob):
        ...
        while True:
            ...
            top_preds, top_words = output[0].sort(descending=True)
            prob_sum = 0
            top_k = 0
            for p in top_preds:
                prob_sum += p
                top_k += 1
                if prob_sum > cumulative_prob:
                    break

            indices = torch.multinomial(top_preds[:top_k], beam_size)
            top_preds = top_preds[indices]
            top_words = top_words[indices]
            # if step == 1:
            #     top_preds, top_words = output[0].topk(beam_size, 0, True, True)
    ...
    def temperature_caption(self, img_features, beam_size, temperature):
        ...
        while True:
            ...
            output = self.deep_output(h)
            # output = top_preds.expand_as(output) + output
            output = nn.functional.softmax(output / temperature)

            # top_preds, top_words = output[0].topk(k, 0, True, True)
            top_words = torch.multinomial(output[0], beam_size)
            top_preds = output[0][top_words]
            # if step == 1:
            #     top_preds, top_words = output[0].topk(beam_size, 0, True, True)
            # else:
            #     top_preds, top_words = output.view(-1).topk(beam_size, 0, True, True)
